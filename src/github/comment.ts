...
function buildSummaryRow(category: string, counts: DiffCounts): string {
  if (counts.total === 0) {
    return `| ${category} | ✅ Synced | 0 |`;
  }

  const details: string[] = [];
  if (counts.onlyInDev > 0) details.push(`🆕 ${counts.onlyInDev}`);
  if (counts.onlyInPrd > 0) details.push(`🗑️ ${counts.onlyInPrd}`);
...
function buildEdgeFunctionsSection(
  ...
  if (diff.onlyInDev.length > 0) {
    lines.push(`#### 🆕 Only in Dev (${diff.onlyInDev.length})`);
    lines.push('');
  ...
  if (diff.different.length > 0) {
    lines.push(`#### 🔄 Changed (${diff.different.length})`);
    lines.push('');
...
function categorizeChange(diff: string): string {
  if (diff.includes('column')) return '📊 Column';
  if (diff.includes('index')) return '📇 Index';
  if (diff.includes('constraint')) return '🔗 Constraint';
  return '📝 Other';
}
